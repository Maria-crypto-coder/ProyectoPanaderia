...
namespace ProyectoPanaderia
{
		class Program 
		{
			static Panaderia panaderias = new Panaderia();
			...
			public static void BorrarServ()
			{
				...
				for (int i = 0; i < panaderias.ListaServicios.Count; i++)
				{
					if (panaderias.ListaServicios[i].Nombre == nombre)
					{
						panaderias.ListaServicios.RemoveAt(i);
						encontrar = true;
						break;
					}
				}
			...
			public static void AgregarUnServicio()
			{
		        ...
		        panaderias.ListaServicios.Add(s);
		    ...
		    public static void TomarUnPedido() {
		        ...
		        if (c == null) {
		            ...
		            panaderias.ListaClientes.Add(c);
		        }
		...
		if(PedidosS.Count == 0) {
		        ...
		        Pedido p = new Pedido(panaderias.ContadorP, c, fecha, PedidosS, costoTotal, seña, costoTotal - seña);
		        panaderias.ListaPedidos.Add(p);
		        panaderias.ContadorP++;
			...
			public static Pedido BuscarP(int codigo) {
		        foreach (Pedido p in panaderias.ListaPedidos) {
		            if (p.Cod == codigo) {
		                return p;
		            }
		        }
		...
		public static void BajarPedido()
			{
			    ...
			    if (p != null)
			    {
			        ...
			        {
			        	panaderias.ListaPedidos.Remove(p);
			        	Console.WriteLine("eliminado");
			        }
			        else
			        {
			...
			{
		        foreach (Servicio s in panaderias.ListaServicios) {
		    ...
		    public static Cliente BuscarC(int dni) {
		        foreach (Cliente c in panaderias.ListaClientes) {
		            if (c.Dni == dni) {
		                return c;
		            }
		        }
